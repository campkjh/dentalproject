...
const pointTypeBadge: Record<PointType, string> = {
  '적립': 'bg-purple-100 text-purple-700',
  '사용': 'bg-orange-100 text-orange-700',
};

const couponStats = [
  { label: '총 발급', value: '1,245', icon: Gift, color: 'bg-purple-500' },
...
const pointStats = [
  { label: '총 발행 포인트', value: '2,450,000P', icon: Coins, color: 'bg-purple-500' },
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <div className="flex items-center gap-3">
              ...
              <button className="flex items-center gap-2 px-4 py-2.5 bg-[#8037FF] text-white rounded-lg text-sm font-medium hover:bg-[#6D28D9] transition-colors shadow-sm">
                <Plus size={16} />
                새 쿠폰 생성
              </button>
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredCoupons.map((coupon) => (
                    <tr key={coupon.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-5 py-4 text-sm text-[#8037FF] font-mono font-medium">{coupon.code}</td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-14 bg-gray-100 rounded-full h-1.5 overflow-hidden">
                            <div
                              className="h-full bg-[#8037FF] rounded-full"
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button className="flex items-center gap-2 px-4 py-2 bg-[#8037FF] text-white rounded-lg text-sm font-medium hover:bg-[#6D28D9] transition-colors shadow-sm">
                <Save size={16} />
                정책 저장
              </button>
            </div>
                  ...
                  {mockPointHistory.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm font-medium">
                        <span className={item.type === '적립' ? 'text-purple-600' : 'text-orange-600'}>
